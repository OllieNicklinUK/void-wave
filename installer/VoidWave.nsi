...
Section "VST3 Plugin (required)" SecVST3
    SectionIn RO   ; can't be deselected

    ; Copy the entire VST3 bundle into Common Files\VST3\
    SetOutPath "$COMMONFILES64\VST3"
    File /r "..\build\VoidWave_artefacts\Release\VST3\Void Wave.vst3"
...
Section "Standalone Application" SecStandalone
    SetOutPath "$INSTDIR"
    File "..\build\VoidWave_artefacts\Release\Standalone\Void Wave.exe"

    ; Start menu shortcut
    CreateDirectory "$SMPROGRAMS\${PRODUCT_PUBLISHER}"
    CreateShortcut  "$SMPROGRAMS\${PRODUCT_PUBLISHER}\${PRODUCT_NAME}.lnk" \
                    "$INSTDIR\Void Wave.exe"

    ; Desktop shortcut (optional but useful)
